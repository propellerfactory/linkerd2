...
| `global.proxy.waitBeforeExitHTTPPath`       | The proxy sidecar will stay alive until requests to the given HTTP path stop reporting success before receiving SIGTERM signal from Kubernetes but no longer than pod's `terminationGracePeriodSeconds`.             | ``                                  |
| `global.proxy.waitBeforeExitHTTPPort`       | The proxy sidecar will stay alive until requests to the given HTTP port stop reporting success before receiving SIGTERM signal from Kubernetes but no longer than pod's `terminationGracePeriodSeconds`.             | `80`                                  |
